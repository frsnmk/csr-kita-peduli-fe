...
export const fetchFeaturedProgram = async (): Promise<Program[]> => {
  try {
    const response = await axiosInstance.get<{data: Program[]}>(
      `${API_URL}programs`,
      {
        params: {
          search: "",
...
export const getProgramDonor = async (
  id: string,
  queryParam?: DonationQueryParams
): Promise<Donation[]> => {
  try {
    let filledQueryParam: any = "";
    if (queryParam) {
      filledQueryParam = Object.fromEntries(
        Object.entries(queryParam).filter(
    ...
    const response = await axiosInstance.get<{data: Donation[]}>(
      `${API_URL}programs/${id}/donations`,
      {
...
export const getProgramPackagePrice = async (
  id: string
): Promise<ProgramPrice[]> => {
  try {
    const response = await axiosInstance.get<{data: ProgramPrice[]}>(
      `${API_URL}programs/${id}/prices`
    );
    return response.data.data;
  } catch (error) {
